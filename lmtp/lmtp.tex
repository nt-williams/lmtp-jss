...
i.i.d. copies of \(Z\) with distribution \(\Prob\). If right-censoring
...
\(H_t\) used for estimation to those at \(t - 1\). The user can inspect
...
Under the augmented data set, the data structure at time \(t\) is
redefined as
...
binary exposure, two pre-specified shift functions are installed with
...
any single model included within it.
...
Discuss that candidate learners which use cross-validation should accept
an id argument for \code{lrnrs_trt}.
...
repository, \pkg{sl3} is not required to use \pkg{lmtp}. Instead, users
may specify the \code{lrnrs_trt} and \code{lrnrs_outcomes} arguments as
...
Observational data is often accompanied with loss-to-follow-up. For this
...
\(t = 3\). Loss-to-follow-up is present after time \(t = 1\) so the data
contains censoring indicators (\code{C1}, \code{C2}). This data set is
installed with the package and is stored in the object \code{sim_cens}.

Suppose we are interested in the additive effect of a MTP where exposure
