...
and $d(A)=0$. The \pkg{lmtp} package presented in this article allows
...
Modified treatment policies are specified using the \code{shift} argument, which
...
repository, it is not required to use \pkg{lmtp}. Instead, the
\code{lrnrs_trt} and \code{lrnrs_outcomes} arguments can be set equal
to \code{NULL} and nuisance parameters will be estimated using a
...
be set with the \code{folds} argument; the minimum number of allowed
...
Dynamic treatment regimes are treatment rules where treatment is applied based on a fixed rule that depends on covariate history. The \pkg{lmtp} package is capable of estimating the effects of both deterministic dynamic treatment rules as well as dynamic modified treatment policies. 
